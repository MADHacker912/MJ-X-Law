...
def _linux_get_display() -> str | None:
    try:
        r = subprocess.run(["xrandr", "--verbose"], capture_output=True, text=True, timeout=3)
...
def _linux_get_brightness() -> float:
    try:
        r = subprocess.run(["xrandr", "--verbose"], capture_output=True, text=True, timeout=3)
        for line in r.stdout.splitlines():
            if "Brightness:" in line:
                val = float(line.split("Brightness:")[1].strip())
                if val > 0.05:
                    return val
    except Exception:
        pass
...
def _linux_set_brightness(val: float) -> None:
    val = max(0.1, min(1.0, float(val)))
    disp = _linux_get_display()
    if disp:
        subprocess.run(["xrandr", "--output", disp, "--brightness", str(round(val, 2))], capture_output=True)
    elif subprocess.run(["which", "brightnessctl"], capture_output=True).returncode == 0:
        percent = int(val * 100)
...
def volume_up():
    if _OS == "Windows":
        ...
        if subprocess.run(["which", "wpctl"], capture_output=True).returncode == 0:
            subprocess.run(["wpctl", "set-volume", "-l", "1.5", "@DEFAULT_AUDIO_SINK@", "10%+"], capture_output=True)
        elif subprocess.run(["which", "amixer"], capture_output=True).returncode == 0:
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        ...
        if subprocess.run(["which", "wpctl"], capture_output=True).returncode == 0:
            subprocess.run(["wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", f"{value / 100.0:.2f}"], capture_output=True)
        elif subprocess.run(["which", "amixer"], capture_output=True).returncode == 0:
            subprocess.run(["amixer", "sset", "Master", f"{value}%"], capture_output=True)
        elif subprocess.run(["which", "pactl"], capture_output=True).returncode == 0:
...
def brightness_up():
    if _OS == "Darwin":
        ...
        cur = _linux_get_brightness()
        _linux_set_brightness(min(1.0, cur + 0.1))
    else:
        try:
            subprocess.run(
                ["powershell", "-Command",
...
def brightness_down():
    if _OS == "Darwin":
        ...
        cur = _linux_get_brightness()
        _linux_set_brightness(max(0.1, cur - 0.1))
    else:
        try:
            subprocess.run(
                ["powershell", "-Command",
...
def brightness_set(value: int):
    ...
    if _OS == "Darwin":
        ...
        _linux_set_brightness(value / 100.0)
    else:
        try:
            subprocess.run(
                ["powershell", "-Command",
...
ACTION_MAP: dict[str, callable] = {
    ...
    "brightness_increase": brightness_up,
    "increase_brightness": brightness_up,
    "brighter":            brightness_up,
    "brightness_down":     brightness_down,
    "brightness_decrease": brightness_down,
...
def computer_settings(
    ...
    if action in ("brightness_set", "set_brightness"):
        try:
            brightness_set(int(value or 80))
