...
def _parse_date(raw: str) -> str:
    ...
    relative = {
        ...
        "tomorrow": today + timedelta(days=1), "kal": today + timedelta(days=1),
        "day after tomorrow": today + timedelta(days=2), "parson": today + timedelta(days=2),
    }
...
def _build_google_flights_url(
    ...
    if return_date:
        trip = f"{origin} to {destination} on {date} returning {return_date}"
    else:
        trip = f"{origin} to {destination} on {date}"

    # tfs/encoded query params are unreliable across sessions and can point at
    # stale dates or routes — Google Flights also parses plain "q" fine.
    return (
        f"{base}"
        f"?q={quote_plus(trip)}"
        f"&curr=USD"
