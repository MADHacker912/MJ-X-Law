"""
file_processor.py — MJ Universal File Processor
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    if action not in prompt_map:
        # Preserve the user's actual instruction instead of overwriting it
        prompt_map["custom"] = f"{instruction}\n\n{content[:40000]}" if instruction else f"{action}\n\n{content[:40000]}"
        action = "custom"
...
def _process_data(path: Path, file_type: str, action: str,
    ...
    if action == "sort":
        ...
        if isinstance(asc, str):
            asc = asc.strip().lower() not in ("0", "false", "no", "off", "")
        try:
