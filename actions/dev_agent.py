...
PROJECTS_DIR     = Path.home() / "Desktop" / "MJProjects"
MAX_FIX_ATTEMPTS = 5
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    """Attempts to automatically install missing packages when ModuleNotFoundError occurs."""
    pattern = re.compile(
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "mj_project")
